...
class EditProduct:
    def __init__(self, product):
        ...
        button = tk.Button(frame1, text="Edytuj", command=lambda: db.update("product", "productName", entry1.get(), "productName", product))
        button.grid(row=0, column=2)

        tk.Label(frame1, text="Tytuł: ").grid(row=1, column=0)
        entry2 = ttk.Entry(frame1)
        entry2.grid(row=1, column=1)
        button = tk.Button(frame1, text="Edytuj", command=lambda: db.update("product", "title", entry1.get(), "productName", product))
        button.grid(row=1, column=2)

        tk.Label(frame1, text="Cena: ").grid(row=2, column=0)
        entry3 = ttk.Entry(frame1)
        entry3.grid(row=2, column=1)
        button = tk.Button(frame1, text="Edytuj", command=lambda: db.update("product", "price", entry1.get(), "productName", product))
        button.grid(row=2, column=2)

        tk.Label(frame1, text="Opis: ").grid(row=3, column=0)
        entry4 = ttk.Entry(frame1)
        entry4.grid(row=3, column=1)
        button = tk.Button(frame1, text="Edytuj", command=lambda: db.update("product", "description", entry1.get(), "productName", product))
