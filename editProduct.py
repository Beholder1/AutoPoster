...
class EditProduct:
    def __init__(self, product):
        ...
        button = tk.Button(frame1, text="Edytuj", command=lambda: db.update("product", "title", entry2.get(), "productName", self.product))
        button.grid(row=1, column=2)

        tk.Label(frame1, text="Cena: ").grid(row=2, column=0)
        entry3 = ttk.Entry(frame1)
        entry3.grid(row=2, column=1)
        button = tk.Button(frame1, text="Edytuj", command=lambda: db.update("product", "price", entry3.get(), "productName", self.product))
        button.grid(row=2, column=2)

        tk.Label(frame1, text="Opis: ").grid(row=3, column=0)
        entry4 = ttk.Entry(frame1)
        entry4.grid(row=3, column=1)
        button = tk.Button(frame1, text="Edytuj", command=lambda: db.update("product", "description", entry4.get(), "productName", self.product))
