...
class EditProduct:
    def __init__(self, product, db):
        ...
        whole_product = db.find_product_by_name(self.product)
        ...
        def execute_adding(button2, entry6, label2):
            ...
            db.save_categories_for_products(product_id, db.find_category(entry6.get()))
            g = db.find_category(entry6.get())
            button2.configure(text="Edytuj",
                              command=lambda: db.update_category(db.find_category(entry6.get()), str(product_id),
                                                                 str(g)))
        ...
        def execute_delete(category_to_delete, button_td, entry_td, button1_td, label_td):
            db.delete_categories_for_products_by_id(str(product_id), str(category_to_delete))
            button_td.destroy()
            entry_td.destroy()
            button1_td.destroy()
            label_td.destroy()
        ...
        ttk.Label(frame1, text="Nazwa: ", background="white", foreground="black", font=('Verdana', 12)).grid(row=0,
                                                                                                             column=0)
        ttk.Label(frame1, text=whole_product[1], background="white", foreground="black", font=('Verdana', 12)).grid(
            row=0, column=1)
        ...
        ttk.Label(frame1, text="Tytuł: ", background="white", foreground="black", font=('Verdana', 12)).grid(row=1,
                                                                                                             column=0)
        ttk.Label(frame1, text=whole_product[2][0: 15] + "...", background="white", foreground="black",
        ...
        ttk.Label(frame1, text="Cena: ", background="white", foreground="black", font=('Verdana', 12)).grid(row=2,
                                                                                                            column=0)
        ttk.Label(frame1, text=whole_product[3], background="white", foreground="black", font=('Verdana', 12)).grid(
            row=2, column=1)
        ...
        ttk.Label(frame1, text="Opis: ", background="white", foreground="black", font=('Verdana', 12)).grid(row=3,
                                                                                                            column=0)
        ttk.Label(frame1, text=whole_product[4][0: 15] + "...", background="white", foreground="black",
        ...
        product_id = whole_product[0]
        categories = db.find_all_product_categories_by_product_id(product_id)
        self.counter = 1
