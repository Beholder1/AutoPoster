...
class EditAccount:
    def __init__(self, account):
        ...
        button = tk.Button(frame1, text="Edytuj", command=lambda: db.update("parts", "email", entry2.get(), "name", self.account))
        button.grid(row=1, column=2)

        tk.Label(frame1, text="Hasło: ").grid(row=2, column=0)
        entry3 = ttk.Entry(frame1)
        entry3.grid(row=2, column=1)
        button = tk.Button(frame1, text="Edytuj", command=lambda: db.update("parts", "password", entry3.get(), "name", self.account))
