...
class RefreshPage:
    def __init__(self, root, db, bgColor, menuColor, activeColor):
        ...
        def chooseNextStep(skipChoosingAccounts):
            if skipChoosingAccounts == 1:
                ...
                for i in db.fetch("parts", "name"):
                    accounts.append(i[0])
                refreshScript.RefreshScript(db, accounts, var3.get())
            else:
                chooseAccounts.ChooseAccountsForRefresh(db, int(numberOfAccounts.get()), var3.get())
        ...
        runButton.grid(row=3, column=1, sticky="w")
